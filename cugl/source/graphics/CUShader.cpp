...
bool Shader::compile() {
    ...
    for(int ii = 0; ii < _fragSource.size(); ii++) {
        if (_fragSource[ii] < 10 || _fragSource[ii]> 127) {
            CULogError("Invalid char at %d: %d",ii,(int)_fragSource[ii]);
        }
    }
...
void Shader::setUniformBlock(GLint pos, const std::shared_ptr<UniformBuffer>& buffer) {
    ...
    for(auto it = _uniblockfields.begin(); it != _uniblockfields.end(); ++it) {
        if (it->second == pos) {
            ...
            if (offset == UniformBuffer::INVALID_OFFSET) {
