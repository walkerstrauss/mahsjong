...
//
#include <cugl/graphics/loaders/CUGradientLoader.h>
 ...
 */
std::shared_ptr<Gradient> GradientLoader::preload(const std::string key, const std::string source) {
 ...
 * @return the loaded gradient
 */
std::shared_ptr<Gradient> GradientLoader::preload(const std::shared_ptr<JsonValue>& json) {
    if (json->isString()) {
        return preload(json->key(),json->asString());
    }
    return Gradient::allocWithData(json);
}
 ...
 */
bool GradientLoader::materialize(const std::string key,
                                 const std::shared_ptr<Gradient>& gradient,
                                 LoaderCallback callback) {

    bool success = gradient != nullptr;
    if (success) {
        _assets[key] = gradient;
    }
 ...
 */
bool GradientLoader::read(const std::string key, const std::string source,
    ...
    if (_loader == nullptr || !async) {
        enqueue(key);
        std::shared_ptr<Gradient> gradient = preload(key,source);
        success = materialize(key, gradient, callback);
    } else {
        _loader->addTask([=,this](void) {
            this->enqueue(key);
            std::shared_ptr<Gradient> gradient = this->preload(key,source);
            Application::get()->schedule([=,this](void){
                this->materialize(key,gradient,callback);
                return false;
            });
        });
    }
 ...
 * @return true if the asset was successfully loaded
 */
bool GradientLoader::read(const std::shared_ptr<JsonValue>& json,
                            LoaderCallback callback, bool async) {
    ...
    if (_loader == nullptr || !async) {
        enqueue(key);
        std::shared_ptr<Gradient> gradient = preload(json);
        success = materialize(key,gradient,callback);
    } else {
        _loader->addTask([=,this](void) {
            this->enqueue(key);
            std::shared_ptr<Gradient> gradient = this->preload(json);
            Application::get()->schedule([=,this](void){
                this->materialize(key,gradient,callback);
                return false;
            });
