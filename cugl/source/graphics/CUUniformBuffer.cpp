...
//  in OpenGL. Unlike a vertex buffer, uniform buffers are optional (you can set the 
//  uniforms directly in the shader). Uniform buffers are solely a performance 
//  optimization. In our tests, they only provide a win when (1) there are a large 
//  number of uniforms being passed to the shader, (2) these uniforms can be loaded at
//  the start of the frame, and (3) shader updates are done through offset management.
//
