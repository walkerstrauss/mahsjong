 ...
 */
bool Texture::initWithData(const void *data, int width, int height, bool mipmaps, Texture::PixelFormat format) {
 ...
 * @return true if initialization was successful.
 */
bool Texture::initWithFile(const std::string filename, bool mipmaps) {
    ...
    bool result = initWithData(normal->pixels, normal->w, normal->h, mipmaps);
...
const std::shared_ptr<Texture>& Texture::getBlank() {
    if (_blank == nullptr) {
        _blank = Texture::allocWithData(cu_2x2_white_image, 2, 2, false);
        _blank->bind();
