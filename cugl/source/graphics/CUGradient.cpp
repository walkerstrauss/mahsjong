...
/**
 * Returns the Vec2 equivalent to a JSON entry
 *
 * @param json  The json entry
 *
 * @return the Vec2 equivalent to a JSON entry
 */
static Vec2 decode_point(JsonValue* json) {
    if (json == nullptr) {
        return Vec2::ZERO;
    }
    
    CUAssertLog(json->size() >= 2, "'%s' must have at least two numbers", json->key().c_str());
    Vec2 result;
    result.x = json->get(0)->asFloat(0.0f);
    result.y = json->get(1)->asFloat(0.0f);
    return result;
}
 ...
 *
 * For a linear gradient, the "center" entry should be replaced with "start" and
 * the "extent" should be replaced with "end". All other values are ignored.
 *
 * For a radial gradient, the "extent" defines the outer radius (according to
 * its distance from the center) while the "radius" is the radius of the inner
 * color. If radius is not specified, then the inner and outer radius are the
 * same.
 *
 * Finally for box gradients, the "center" is the center, while the "extent"
 * defines the width and height. All other values are interpretted normally.
 *
...
bool Gradient::initWithData(const std::shared_ptr<JsonValue>& data) {
    ...
    Color4 inner;
    if (data->has("inner")) {
        inner.set(json_color(data->get("inner").get(),"#ffffff"));
    }
    Color4 outer;
    if (data->has("outer")) {
        outer.set(json_color(data->get("outer").get(),"#ffffff"));
    } else {
        outer = inner;
    }
    ...
    if (!data->has("type")) {
        ...
        if (type == "linear") {
            Vec2 start = decode_point(data->get("start").get());
            Vec2 end = decode_point(data->get("end").get());
            return initLinear(inner, outer, start, end);
        } else {
            Vec2 center = decode_point(data->get("center").get());
            Vec2 extent = decode_point(data->get("center").get());
            if (type == "radial") {
