...
#pragma mark -
#pragma mark TextLayout::Glyph Debugging
/**
 ...
 */
Mesh<SpriteVertex> TextLayout::getGlyphBoxes() const {
    Mesh<SpriteVertex> result;
    result.command = GL_LINES;
    getGlyphBoxes(result);
    return result;
}
 ...
 * @return a (line) mesh of the quad outlines for the text glyphs
 */
Mesh<SpriteVertex> TextLayout::getGlyphBoxes(Rect rect) const {
    Mesh<SpriteVertex> result;
    result.command = GL_LINES;
    getGlyphBoxes(result,rect);
    return result;
}
 ...
 */
size_t TextLayout::getGlyphBoxes(Mesh<SpriteVertex>& mesh) const {
    Rect bounds = getBounds();
    size_t total = 0;
    for(auto it = _rows.begin(); it != _rows.end(); ++it) {
        float track = 0;
        ...
        const char* begin = _text.c_str()+it->begin;
        const char* end = _text.c_str()+it->end;
        total += _font->getGlyphBoxes(mesh, begin, end, it->exterior.origin, bounds, track);
    }
    return total;
}
 ...
 * @return the number of quads generated
 */
size_t TextLayout::getGlyphBoxes(Mesh<SpriteVertex>& mesh, Rect rect) const {
    Rect bounds = getBounds();
    bounds.intersect(rect);
    size_t total = 0;
    for(auto it = _rows.begin(); it != _rows.end(); ++it) {
...
void TextLayout::breakLines() {
    ...
    while (curr != textEnd) {
        ...
        if (type == UnicodeType::NEWLINE) {
            ...
            row->interior.size.width = (wordMaxX == 0) ? lineWidth : wordMaxX-row->interior.origin.x;
            ...
            if (rowStart) {
                ...
                if (_breakline > 0 && (type == UnicodeType::CHAR || type == UnicodeType::CJK) && nextWidth > _breakline) {
                    ...
                    if (wordBegin == rowBegin) {
                        ...
                        // Check if we can squeeze this one in (only makes sense if left aligned)
                        bool left = _halign == HorizontalAlign::HARD_LEFT || _halign == HorizontalAlign::LEFT;
                        bool squeeze = *next == '\0' && left;
