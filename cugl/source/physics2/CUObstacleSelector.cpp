...
void ObstacleSelector::updateTarget(Obstacle* obstacle) {
    if (obstacle) {
        ...
        if (_connect != nullptr) {
            ...
            Path2 path;
            path.vertices.push_back(pos1);
            path.vertices.push_back(pos2);
            _connect->setPath(path);
...
void ObstacleSelector::resetDebug() {
    bool hviz = _hatch == nullptr ? false : _hatch->isVisible();
    bool cviz = _connect == nullptr ? false : _connect->isVisible();
    ...
    _hatch = scene2::WireNode::allocWithTraversal(verts, 4, indx, 12);
    _hatch->setColor(_dcolor);
    _hatch->setVisible(hviz);

    _connect = scene2::WireNode::alloc();
    _connect->setColor(_dcolor);
    _connect->setAbsolute(true);
    _connect->setVisible(cviz);
