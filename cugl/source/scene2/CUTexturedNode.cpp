 ...
 */
bool TexturedNode::initWithData(const AssetManager* manager,
    ...
    setTexture(manager->get<Texture>(data->getString("texture",UNKNOWN_STR)));
    
    // Set the other properties
    if (data->has("gradient")) {
        auto child = data->get("gradient");
        if (child->isString()) {
            _gradient = manager->get<Gradient>(child->asString());
        } else {
            _gradient = Gradient::allocWithData(child);
        }
    }
    _absolute = data->getBool("absolute",false);
    _blendEquation = gl_blend_eq(data->getString("blendeq","GL_FUNC_ADD"));
    _srcFactor = gl_blend_func(data->getString("blendsrc","GL_SRC_ALPHA"));
    _dstFactor = gl_blend_func(data->getString("blenddst","GL_ONE_MINUS_SRC_ALPHA"));
