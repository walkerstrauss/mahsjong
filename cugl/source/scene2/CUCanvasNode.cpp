...
//
#include <cugl/scene2/CUCanvasNode.h>
    ...
    void materialize(CommandType ctype) {
        ...
        if (ctype == CommandType::TEXT) {
                    ...
                    switch(ctype) {
                        ...
                        case CommandType::MASK_FILL:
                        {
                            ...
                            for(auto jt = path->vertices.begin(); jt != path->vertices.end(); ++jt) {
                                packet->mesh.vertices.push_back(SpriteVertex());
                                SpriteVertex* vert = &(packet->mesh.vertices.back());
                                vert->position = *jt;
                                vert->color = rgba;
                            }
            ...
            switch(packet->type) {
                ...
                {
                    SpriteVertex* vert;
                    packet->mesh.vertices.emplace_back();
 ...
 * @return true if initialization was successful.
 */
bool CanvasNode::initWithData(const AssetManager* manager,
