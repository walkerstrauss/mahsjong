...
bool ProgressBar::initWithData(const AssetManager* manager,
    ...
    std::shared_ptr<Texture> background = manager->get<Texture>(temp);
    ...
    std::shared_ptr<Texture> beginCap = manager->get<Texture>(temp);
    if (beginCap != nullptr) {
        scale = size/beginCap->getSize();
        scale.x = 1.0f;
        _begincap = PolygonNode::allocWithTexture(beginCap);
    ...
    std::shared_ptr<Texture> finalCap = manager->get<Texture>(temp);
    if (finalCap != nullptr) {
        scale = size/finalCap->getSize();
        scale.x = 1.0f;
        _finalcap = PolygonNode::allocWithTexture(finalCap);
    ...
    std::shared_ptr<Texture> foreground = manager->get<Texture>(temp);
    if (foreground != nullptr) {
        scale = size/foreground->getSize();
        scale.x = 1.0f;
        _foreground = PolygonNode::allocWithTexture(foreground);
