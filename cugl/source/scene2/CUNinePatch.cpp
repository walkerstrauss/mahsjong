...
//
#include <cugl/scene2/CUNinePatch.h>
 ...
 */
bool NinePatch::initWithData(const AssetManager* manager,
...
void NinePatch::generateRenderData() {
    ...
    // Bottom left corner
    if (_interior.origin.x > 0 && _interior.origin.y > 0) {
        dst.origin.x = 0;
        dst.origin.y = 0;
...
Uint32 NinePatch::generatePatch(const Rect src, const Rect dst, Uint32 offset) {
    ...
    SpriteVertex temp;
    temp.color = Color4::WHITE.getPacked();
 ...
 */
void NinePatch::draw(const std::shared_ptr<SpriteBatch>& batch, const Affine2& transform, Color4 tint) {
    if (!_rendered) {
        generateRenderData();
    }
