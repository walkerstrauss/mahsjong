...
#include <cugl/scene2/CUTextField.h>
...
// TODO: Profile slowdown on initial click
 ...
 */
bool TextField::initWithData(const AssetManager* manager, const std::shared_ptr<JsonValue>& data) {
    if (Label::initWithData(manager, data)) {
...
bool TextField::activate() {
    ...
    if (mouse) {
    	...
    	check = mouse->addPressListener(_fkey, [=,this](const MouseEvent& event, Uint8 clicks, bool focus) {
    		updatePress(event.position, focus);
    	});
    } else {
    	_mouse = false;
        if (!_fkey) { _fkey = touch->acquireKey(); }
    	check = touch->addBeginListener(_fkey, [=,this](const TouchEvent& event, bool focus) {
    		updatePress(event.position, focus);
    	});
    }
    if (!check) {
