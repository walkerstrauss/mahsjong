 ...
 */
bool PathNode::initWithData(const AssetManager* manager,
...
void PathNode::generateRenderData() {
    ...
    if (nsize != bsize) {
        adjust = true;
        shift.scale((bsize.width > 0 ? nsize.width/bsize.width : 1),
                    (bsize.height > 0 ? nsize.height/bsize.height : 1));
    }
...
void PathNode::updateExtrusion() {
    ...
    if (_stroke > _fringe) {
        ...
        for (auto it = _path.vertices.begin(); it != _path.vertices.end(); ++it) {
            ...
            SpriteVertex* vert = &_mesh.vertices.back();
            vert->position = *it;
            vert->color = white;
        }
