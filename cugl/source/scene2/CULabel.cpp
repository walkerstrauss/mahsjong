 ...
 * @return true if initialization was successful.
 */
bool Label::initWithData(const AssetManager* manager, const std::shared_ptr<JsonValue>& data) {
    if (_font != nullptr) {
        CUAssertLog(false, "Label is already initialized");
        return false;
    } else if (!data) {
    ...
    // Set the font
    auto font = manager->get<Font>(data->getString("font",UNKNOWN_STR));
    if (font == nullptr) {
        CUAssertLog(false, "The font is undefined");
        return false;
    }
    ...
    if (data->has("foreground")) {
        _foreground.set(json_color(data->get("foreground").get(),"#ffffff"));
    }
    
    if (data->has("background")) {
        _background.set(json_color(data->get("background").get(),"#00000000"));
    }
