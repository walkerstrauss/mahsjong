...
//
#include <cugl/scene2/CUScrollPane.h>
 ...
 * @return true if initialization was successful.
 */
bool ScrollPane::initWithData(const AssetManager* manager,
                              const std::shared_ptr<JsonValue>& data) {
    if (SceneNode::initWithData(manager, data)) {
 ...
 */
Affine2 ScrollPane::getNodeToWorldTransform() const {
    Affine2 result;
    Affine2::multiply(_combined, _panetrans, &result);
    if (_parent) {
        // Multiply on left
 ...
 */
bool ScrollPane::inContentBounds(const Vec2 point) {
    Vec2 local = _panetrans.transform(screenToNodeCoords(point));
