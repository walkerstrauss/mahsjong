...
Uint32 AudioWaveNode::read(float* buffer, Uint32 frames) {
    ...
    ATK_VecScale(buffer, _ndgain.load(std::memory_order_relaxed), buffer,read*_channels);
    if (read > 0) {
...
std::shared_ptr<AudioWaveform> AudioWaveform::allocWithData(const std::shared_ptr<JsonValue>& data) {
    ...
    shape = cugl::strtool::tolower(shape);
    ...
    if (_newfreq.load(std::memory_order_relaxed)) {
        ...
        if (type == Type::NOISE) {
            ...
            if (freq < 0) {
                ...
                Uint32 seed = (Uint32)(std::min(freq,1.0f)*(float)std::numeric_limits<Uint32>::max());
                _random.seed(seed);
            }
        }
    }
