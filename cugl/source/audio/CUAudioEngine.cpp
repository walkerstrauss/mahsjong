...
bool AudioEngine::init(const std::shared_ptr<AudioOutput>& device, Uint32 slots) {
    ...
    for(int ii = 0; ii <= _capacity; ii++) {
        ...
        if (ii < _capacity) {
            channel->setCallback([=,this](const std::shared_ptr<cugl::audio::AudioNode>& node,
 ...
 *
 * This method keeps track of all sounds in-flight for the given key,
 * ensuring proper cleanup afterwords.
 *
 * @param key   The for the audio node
 * @param fader The node to attach to the key
 */
void AudioEngine::addKey(const std::string key, const std::shared_ptr<audio::AudioFader>& fader) {
 ...
 *
 * @param key  The key to purge from the list of active effects.
 */
void AudioEngine::removeKey(const std::string key) {
    _actives.erase(key);
    for(auto it = _priority.begin(); it != _priority.end(); ) {
        if (*it == key) {
            it = _priority.erase(it);
            break;
        } else {
            it++;
        }
 ...
 *
 * @return the active slot for the given key
 */
int AudioEngine::getSlot(const std::string key) {
    auto it = _actives.find(key);
    if (it != _actives.end()) {
...
void AudioEngine::gcollect(const std::shared_ptr<audio::AudioNode>& sound, bool status) {
    ...
    intptr_t id = reinterpret_cast<intptr_t>(sound.get());
    auto it = _evicts.find(id);
    if (it != _evicts.end()) {
        _evicts.erase(id);
    } else {
        auto jt = _actives.find(key);
        if (jt != _actives.end()) {
            tag = jt->second->getTag();
        }
    }
    ...
    if (tag >= 0) {
        auto scheduler = _slots[tag];
        if (!scheduler->isPlaying() && scheduler->getTailSize() == 0) {
            removeKey(key);
        }
    }
...
bool AudioEngine::play(const std::string key, const std::shared_ptr<Sound>& sound,
    ...
    if (audioID == -1) {
        if (force && !_priority.empty()) {
            std::string altkey = _priority.front();
            _priority.pop_front();
            
            audioID = _actives[altkey]->getTag();
            intptr_t aptr = reinterpret_cast<intptr_t>(_actives[altkey].get());
            _evicts.emplace(aptr);
    ...
    fader->setName(key);
    addKey(key, fader);
    _slots[audioID]->append(fader, loop ? -1 : 0);
    return true;
}
...
AudioEngine::State AudioEngine::getState(const std::string key) const {
    ...
    std::shared_ptr<AudioNode> node = it->second;
...
const std::string AudioEngine::getSource(const std::string key) const {
    ...
    std::shared_ptr<AudioNode> source = accessInstance(it->second);
...
bool AudioEngine::isLoop(const std::string key) const {
    ...
    if (it != _actives.end()) {
        ...
        return _slots.at(tag)->getLoops() != 0;
    }
    return false;
...
float AudioEngine::getPanFactor(const std::string key) const {
    ...
    auto it = _actives.find(key);
    if (it != _actives.end()) {
        std::shared_ptr<AudioFader> fader = it->second;
        std::shared_ptr<AudioPanner> panner = std::dynamic_pointer_cast<AudioPanner>(fader->getInput());
...
float AudioEngine::getDuration(const std::string key) const  {
    ...
    auto it = _actives.find(key);
    if (it != _actives.end()) {
        std::shared_ptr<audio::AudioNode> source = accessInstance(it->second);
        AudioPlayer* player = dynamic_cast<AudioPlayer*>(source.get());
...
void AudioEngine::clear(const std::string key,float fade) {
    ...
    auto it = _actives.find(key);
    if (it != _actives.end()) {
        std::shared_ptr<AudioFader> node = it->second;
