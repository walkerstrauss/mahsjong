...
Uint32 AudioMixer::read(float* buffer, Uint32 frames) {
    ...
    if (!_paused.load(std::memory_order_relaxed)) {
        ...
        while (remain > 0) {
            ...
            for(int ii = 0; ii < _width; ii++) {
                ...
                if (temp) {
                    ...
                    ATK_VecAdd(_buffer,output,output,chunk*_channels);
                }
            }
        ...
        ATK_VecScale(buffer,_ndgain.load(std::memory_order_relaxed),buffer,actual*_channels);
        float knee = _knee.load(std::memory_order_relaxed);
        if (knee == 1) {
            ATK_VecClip(buffer,-1,1,buffer,actual*_channels);
        } else if (knee > 0) {
            ATK_VecClipKnee(buffer,1,knee,buffer,actual*_channels);
        }
    } else {
