...
bool AudioSpinner::init(Uint8 channels, Uint8 field, Uint32 rate) {
    if (AudioNode::init(channels,rate)) {
        ...
        _dirtycross = false;
        _filter = ATK_AllocSOFilter(ATK_SO_LOWSHELF,_crossover/rate,1.0f,INV_SQRT2);
        return true;
    }
    return false;
}
            ...
            if (amt == 0) {
                ...
                if (_channels > 4 && cross) {
                    if (dirty) {
                        ...
                        ATK_FreeIIRFilter(_filter);
                        _filter = ATK_AllocSOFilter(ATK_SO_LOWSHELF,_crossover/_sampling,
                                                    1.0f,INV_SQRT2);
                    }
                    if (_field > 1) {
                    ...
                    ATK_ApplyIIRFilter(_filter, _buffer, _buffer, frames);
