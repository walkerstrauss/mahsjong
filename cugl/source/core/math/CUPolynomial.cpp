...
#include <functional>
#include <sstream>
#include <cugl/core/math/CUPolynomial.h>
...
Polynomial& Polynomial::operator*=(float value) {
    if (value == 0) {
        ...
        std::transform(begin(), end(), begin(), [=](float x) { return x*value; });
    }
    return *this;
}
...
Polynomial Polynomial::recursive_multiply(const Polynomial& a, const Polynomial& b) {
    if (a.size() == 1) {
        ...
        float c = a[0];
        std::transform(b.begin(), b.end(), result.begin(), [=](float x) { return x*c; });
        return result;
    } else if (b.size() == 1) {
        Polynomial result(a.degree());
        float c = b[0];
        std::transform(a.begin(), a.end(), result.begin(), [=](float x) { return x*c; });
        return result;
    }
...
Polynomial operator-(float left, const Polynomial& right) {
    ...
    std::transform(right.begin(), right.end(), result.begin(), [=](float x) { return -x; });
    result.back() += left;
    return result;
}
