...
//
#include <cugl/core/math/CUPlane.h>
...
std::string Plane::toString(bool verbose) const {
    ...
    ss << strtool::to_string(normal.x) << "x+";
    ss << strtool::to_string(normal.y) << "y+";
    ss << strtool::to_string(normal.z) << "z = ";
    ss << strtool::to_string(offset);
    ss << "]";
    return ss.str();
}
