...
//
//  This module a modern C++ alternative to the CUJSON interface for reading
//  JSON files.  In particular, this gives us better type-checking and memory
//  management.  With that said, it still uses CUJSON as the underlying parsing
//  engine.
//
 ...
 */
static std::string isolate_error(const char* data, const char* error, int& lineno) {
    lineno = 1;
    const char* pos = data;
    while (pos != error) {
        if (*pos == '\n') {
...
/**
 * Returns the JsonValue::Type appropriate for this CUJSON node
 *
 * CUJSON types are slightly different from JsonValue ones, particular for
 * boolean and strings.  We need this function to convert.
 *
 * @param node  The CUJSON node
 *
 * @return the JsonValue::Type appropriate for this CUJSON node
 */
static JsonValue::Type JsonValueType(const CUJSON* node) {
    if (node->type & CUJSON_False || node->type & CUJSON_True) {
        return JsonValue::Type::BoolType;
    } else if (node->type & CUJSON_Number) {
        return JsonValue::Type::NumberType;
    } else if (node->type & CUJSON_String) {
        return JsonValue::Type::StringType;
    } else if (node->type & CUJSON_NULL) {
        return JsonValue::Type::NullType;
    } else if (node->type & CUJSON_Array) {
        return JsonValue::Type::ArrayType;
    } else if (node->type & CUJSON_Object) {
...
/**
 * Returns the CUJSON type appropriate for this JsonValue
 *
 * CUJSON types are slightly different from JsonValue ones, particular for
 * boolean and strings.  We need this function to convert.
 *
 ...
 *
 * @return the CUJSON type appropriate for this JsonValue
 */
static int CUJSONType(const JsonValue* value) {
    switch (value->type()) {
        case JsonValue::Type::NullType:
            return CUJSON_NULL;
        case JsonValue::Type::BoolType:
            return (value->asBool() ? CUJSON_True : CUJSON_False);
        case JsonValue::Type::NumberType:
            return CUJSON_Number;
        case JsonValue::Type::StringType:
            return CUJSON_String;
        case JsonValue::Type::ArrayType:
            return CUJSON_Array;
        case JsonValue::Type::ObjectType:
            return CUJSON_Object;
    }
...
/**
 * Returns a newly allocated JsonValue equivalent to the CUJSON node
 *
 ...
 *
 * This method does not delete the CUJSON node when done.
 *
 * @param node  The CUJSON node to convert
 *
 * @return a newly allocated JsonValue equivalent to the CUJSON node
 */
std::shared_ptr<JsonValue> JsonValue::toJsonValue(const CUJSON* node) {
    std::shared_ptr<JsonValue> result = JsonValue::alloc(JsonValueType(node));

    if (result->_type == Type::BoolType) {
        result->_longValue = node->type & CUJSON_True;
    } else {
...
/**
 * Modifies value so that it is equivalent to the CUJSON node
 *
 * This allocator recursively allocates child nodes as necessary. These
 ...
 * @param node  The CUJSON node to convert
 */
void JsonValue::toJsonValue(JsonValue* value, const CUJSON* node) {
    value->_type = JsonValueType(node);
    if (value->_type == Type::BoolType) {
        value->_longValue = node->type & CUJSON_True;
    } else {
...
/**
 * Returns a newly allocated CUJSON node equivalent to value
 *
 * This method recursively allocates child nodes as necessary. These
 * nodes will be owned by the parent node and deleted when it is deleted.
 * However, the returned CUJSON node is not stored in a smart pointer, so
 * it must be manually deleted (with {@link CUJSON_Delete}) when it is no
 * longer necessary.
 *
 * @param value The JsonValue to convert
 */
CUJSON* JsonValue::toCUJSON(const JsonValue* value) {
    CUJSON* result;
    switch (value->type()) {
        case JsonValue::Type::NullType:
            result = CUJSON_CreateNull();
            break;
        case JsonValue::Type::BoolType:
            result = CUJSON_CreateBool(value->asBool());
            break;
        case JsonValue::Type::NumberType:
            result = CUJSON_CreateNumber(value->asDouble());
            break;
        case JsonValue::Type::StringType:
            result = CUJSON_CreateString(value->asString().c_str());
            break;
        case JsonValue::Type::ArrayType:
            result = CUJSON_CreateArray();
            break;
        case JsonValue::Type::ObjectType:
            result = CUJSON_CreateObject();
            break;
        default:
            CUAssertLog(false,"Unknown JSON type %d",value->type());
    }
    result->type = result->type | CUJSON_StringIsConst;
    ...
    for(auto it = value->_children.begin(); it != value->_children.end(); ++it) {
        CUJSON* current = toCUJSON(it->get());
        if (first) {
 ...
 * @return  true if the JSON node is initialized properly, false otherwise.
 */
bool JsonValue::initWithJson(const std::string json) {
    const char *error = NULL;
    CUJSON* node = CUJSON_ParseWithOpts(json.c_str(), &error, 0);
    if (node) {
    ...
    if (error) {
        int line = 0;
        std::string source = isolate_error(json.c_str(),error,line);
 ...
 */
void JsonValue::merge(const std::shared_ptr<JsonValue>& node) {
 ...
 * to the file.  Providing this string to the {@link alloCUJSON} constructor
...
std::string JsonValue::toString(bool format) const {
    CUJSON* ast = toCUJSON(this);
    char* data = nullptr;
    if (format) {
        data = CUJSON_Print(ast);
    } else {
        data = CUJSON_PrintUnformatted(ast);
    }
    if (data) {
