...
/**
 * Returns a hexadecimal string for the given 64 bit integer
 *
 * If the value len is longer than the number of hexadecimal digits
 * necessary to represent the value, the result will be padded
 * with leading 0s to reach this length.
 ...
 * @return a hexadecimal string for the given 32 bit integer
 */
std::string to_hexstring(Uint64 value, size_t len) {
    static const char* digits = "0123456789ABCDEF";
 ...
 *
 * The Levenshtein distance is an estimate of the minimum number of
 * insertions, deletions, and/or substitutions to turn one string
 * into the other. It is used to support ComboBox fuzzy search.
 *
 * @param str1   First string
 * @param str2   Second string
 *
 * @return the Levenshtein distance from str1 to str2
 */
int levenshteinDist(std::string str1, std::string str2) {
    std::vector<std::vector<int>> dists;
    for (int i = 0; i < str1.length() + 1; ++i) {
        std::vector<int> row = std::vector<int>(str2.length()+1);
        dists.push_back(row);
    }
    for (int i = 1; i < str1.length() + 1; ++i) {
        for (int j = 1; j < str2.length() + 1; ++j) {
            ...
            int dist = std::min(dists[i - 1][j] + 1,    // delete
                       std::min(dists[i][j - 1] + 1,    // insert
                       dists[i - 1][j - 1]+cost));      // sub
            dists[i][j] = dist;
        }
    }
    return dists[str1.length()][str2.length()];
}
...
/**
 * Returns a byte vector equivalent to the provided string.
 *
 * This function is used for network transmission of binary data.
 *
 * @param str   The string to convert
 *
 * @return a byte vector equivalent to the provided string.
 */
std::vector<std::byte> str2bytes(const std::string str) {
    std::vector<std::byte> result;
    result.reserve(str.size());
    for(auto it = str.begin(); it != str.end(); it++) {
        std::byte bite = static_cast<std::byte>(*it);
...
/**
 * Returns a string equivalent to the given byte vector.
 *
 * Strings in C++ do not have a specific encoding (like Python or Java
 * does). A string is simply a list of characters, and the meaning of those
 * characters depends on the choice of encoding. As such, it makes perfect
 * sense to convert a byte vector to a string.
 *
 * @param data  The byte vector to convert
 *
 * @return a string equivalent to the given byte vector.
 */
std::string bytes2str(const std::vector<std::byte>& data) {
    std::string result;
