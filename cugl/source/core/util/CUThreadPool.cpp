...
/**
 * Stops the thread pool, marking it for shut down.
 *
...
/**
 * Instructs the current thread to sleep for the given number of milliseconds.
 *
 * This method will apply to the current thread, which is not guaranteed to
 * be a member of this thread pool. We provide this method only as a way
 * to abstract thread sleeping from the thread implementation.
 *
 * @param millis    The number of milliseconds to sleep.
 */
void ThreadPool::sleep(Uint32 millis) {
#ifdef CU_SDL_THREADS
    SDL_Delay(millis);
#else
