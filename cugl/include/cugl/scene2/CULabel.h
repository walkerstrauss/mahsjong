 ...
 *
 * The text itself is formatted using a {@link graphics::TextLayout}. This text
 * layout breaks the text into multiple lines as needed, though by default, a
 * label only breaks lines at explicit newlines. The text layout aligns the
 * lines of text with each other. But it also positions the text against the
 * background rectangle as well. See {@link #getHorizontalAlignment} and
 * {@link #getVerticalAlignment} for more information.
 *
 * To display the text, you need a {@link graphics::Font}. The label assumes
 * that the font has an atlas, either through a pre-built atlas, or by setting
 * the attribute {@link graphics::Font#setAtlasFallback} to true. If the font
 * does not have an atlas, or characters are missing from the atlas, then those
 * glyphs will not be displayed. It is generally recommended that you use a
 * prebuilt atlas, as fallback atlases introduce significant time and memory
 * overhead.
 */
...
protected:
    ...
    std::shared_ptr<graphics::TextLayout> _layout;
    ...
    std::unordered_map<GLuint, std::shared_ptr<graphics::GlyphRun>> _glyphrun;
...
#pragma mark Constructors
     ...
     * @return true if initialization was successful.
     */
    bool initWithText(const std::string text, const std::shared_ptr<graphics::Font>& font);
     ...
     * @return true if initialization was successful.
     */
    bool initWithText(const Vec2 position, const std::string text,
     ...
     * @return true if initialization was successful.
     */
    bool initWithTextBox(const Size size, const std::string text,
                         const std::shared_ptr<graphics::Font>& font);
     ...
     * @return true if initialization was successful.
     */
    bool initWithTextBox(const Rect rect, const std::string text,
                         const std::shared_ptr<graphics::Font>& font);
...
#pragma mark Static Constructors
     ...
     */
    static std::shared_ptr<Label> allocWithText(const std::string text,
                                                const std::shared_ptr<graphics::Font>& font) {
        std::shared_ptr<Label> result = std::make_shared<Label>();
     ...
     * @return a newly allocated label with the given text and font atlas
     */
    static std::shared_ptr<Label> allocWithText(const Vec2 position, const std::string text, 
                                                const std::shared_ptr<graphics::Font>& font) {
        std::shared_ptr<Label> result = std::make_shared<Label>();
...
#pragma mark Text Attributes
     ...
     */
    std::shared_ptr<graphics::Font> getFont() const { return _font; }
     ...
     */
    void setFont(const std::shared_ptr<graphics::Font>& font, bool resize=false);
     ...
     * {@link graphics::TextLayout}. But it also represents the relationship
     * between the text and the background region of this node. In particular,
     ...
     */
    graphics::HorizontalAlign getHorizontalAlignment() const;
     ...
     * HARD_LEFT in {@link graphics::TextLayout}. Similarly, RIGHT and
     * HARD_RIGHT place the right edge of the text layout against the right
     * edge of the label, after applying padding.
     *
     * Padding is unusual for the CENTER and TRUE_CENTER alignments. The
     * center of the label is determined from the edges after applying
     * padding. So if the left and right padding are not equal, then
     ...
     */
    void setHorizontalAlignment(graphics::HorizontalAlign halign);
     ...
     *
     * TOP and HARD_TOP place the top edge of the text layout against the top
     * edge of the label, after applying padding. Note that the meaning of "top
     * edge" differs between TOP and HARD_TOP in {@link graphics::TextLayout}.
     ...
     * line (not the top, as the case with {@link graphics::TextLayout}) line
     * at the bottom edge of the label, after adjusting for padding.
     *
     * This value is TOP by default.
     *
     * @return the vertical alignment of the text.
     */
    graphics::VerticalAlign getVerticalAlignment() const;
     ...
     */
    void setVerticalAlignment(graphics::VerticalAlign valign);
...
#pragma mark Label Sizing
     ...
     * This corresponds to {@link graphics::TextLayout#getBounds} of the
     * underlying text layout. In particular, this means there may be some
     * natural spacing around the characters.
     *
     ...
     * corresponds to {@link graphics::TextLayout#getTrueBounds} of the
...
#pragma mark Text Coloring
     ...
     * {@link graphics::Font#getPadding} as the blur size. This will prevent
     * bleeding across characters in the atlas.
     *
