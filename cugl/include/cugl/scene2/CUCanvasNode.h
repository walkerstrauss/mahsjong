...
#include <cugl/graphics/CUTextAlignment.h>
...
public:
     ...
     *
     * Traditional {@link graphics::Gradient} and {@link graphics::Texture}
     * objects use classic texture coordinates, making them somewhat difficult
     * to use. A paint is a more user-friendly approach that allows you to
     * define these elements using positional coordinates. This makes it easier
     * to align gradients or images with the shape that are drawn.
     *
     * For example, suppose a paint is a linear gradient that starts at the
     * origin and ends at position (100,100). If applied to a rectangle at the
     * origin with size 100x100, this gradient will span the entire rectangle, 
     * running along the diagonal. However, if it is applied to a rectangle of
     * size 200x200, it will stop in the center.
     *
     * An important limitation of the canvas node architecture is that any
     * shape can either colored by a gradient or a texture, but not both. You
     * should use a special purpose scene graph node like {@link PolygonNode}
    ...
    private:
         ...
         * {@link graphics::Gradient} or {@link graphics::Texture} object
         * when generating a drawing command.
         */
        enum Type {
    ...
    public:
         ...
         *
         * This initializer is very similar to {@link graphics::Gradient#initLinear},
         ...
         *
         * This initializer is very similar to {@link graphics::Gradient#initRadial},
         * except that the positions are given in the coordinate system of the
         * canvas node, and not using texture coordinates.
         *
         * @param inner     The inner gradient color
         ...
         *
         * This initializer is very similar to {@link graphics::Gradient#initBox},
         * except that the positions are given in the coordinate system of the
         * canvas node, and not using texture coordinates.
         *
         * @param inner     The inner gradient color
         ...
         * texture coordinates.
         *
         * Typically a texture pattern is applied to a shape that fully fits
         * within the bounds. For example, if these bounds are the bounding box
         * of a polygon, and this paint is applied to the polygon, the effect is
         * the same as for a {@link PolygonNode}.
         *
         ...
         * @return true if initialization was successful.
         */
        bool initPattern(std::shared_ptr<graphics::Texture> texture, const Rect bounds) {
         ...
         * @return true if initialization was successful.
         */
        bool initPattern(std::shared_ptr<graphics::Texture> texture, const Vec2 origin, const Size size);
         ...
         * Typically a texture pattern is applied to a shape that fully fits
         * within the bounds. For example, if these bounds are the bounding box
         * of a polygon, and this paint is applied to the polygon, the effect
         * is the same as for a {@link PolygonNode}.
         *
         * @param texture   The texture to paint with
         * @param bounds    The bounding box to place the texture
         *
         ...
         */
        static std::shared_ptr<Paint> allocPattern(std::shared_ptr<graphics::Texture> texture, const Rect bounds) {
            std::shared_ptr<Paint> result = std::make_shared<Paint>();
         ...
         * @return a new texture pattern with the given bounds.
         */
        static std::shared_ptr<Paint> allocPattern(std::shared_ptr<graphics::Texture> texture,
...
#pragma mark Static Constructors
     ...
     */
    static std::shared_ptr<CanvasNode> allocWithData(const AssetManager* manager,
                                                     const std::shared_ptr<JsonValue>& data) {
        std::shared_ptr<CanvasNode> result = std::make_shared<CanvasNode>();
        return (result->initWithData(manager,data) ? result : nullptr);
    }
...
#pragma mark Render State
     ...
     * {@link graphics::Scissor#intersect}. The intersection will take place
     * in the coordinate system of this scissor.
     *
     ...
     * new local scissor by calling {@link graphics::Scissor#intersect} on
     * the previous local scisor. The intersection will take place in the
     * coordinate system of this scissor.
     *
...
#pragma mark Text Commands
     ...
     *
     * When blurring text, use a font with the same {@link graphics::Font#getPadding}
     ...
     *
     * See {@link graphics::HorizontalAlign} for how alignment affects the text
     * origin.
     *
     ...
     *
     * The vertical alignment defines the y-coordinate origin of this text 
     * layout. In the case of multiple lines, the alignment is (often) with
     * respect to the entire block of text, not just the first line.
     *
     * See {@link graphics::VerticalAlign} for how alignment affects the text
     * origin.
     *
