...
#include <cugl/scene2/CULabel.h>
#include <cugl/core/input/CUTextInput.h>
#include <cugl/core/input/CUKeyboard.h>
#include <string>
...
public:
  ...
  /**
  *   This enum represents the 3 types of listeners for a text field
  */
  enum class ListenerType : int {
    /** Listener type for gaining focus of the text field */
    ENTER        = 0,
    /** Listener type for a text update in the text field */
    TYPE         = 1,
    /** Listener type for losing focus of the text field */
    EXIT         = 2
  };
...
#pragma mark Values
    ...
    /** The listener callbacks for recept of focus */
    std::unordered_map<Uint32, Listener> _enterListeners;
...
#pragma mark Static Constructors
    ...
    static std::shared_ptr<SceneNode> allocWithData(const AssetManager* manager,
        ...
        if (!node->initWithData(manager,data)) { node = nullptr; }
...
#pragma mark Listeners
    ...
    /**
     * Returns true if this text field has an enter listener
     *
     * This listener is invoked when the field gains focus.
     *
     * @return true if this text field has an enter listener
     */
    bool hasEnterListener() const { return !_enterListeners.empty(); }
     ...
     * @return the listener for the given key
     */
    const Listener getEnterListener(Uint32 key) const;

    /**
     * Returns all enter listeners for this text field
     *
     * These listeners are invoked when the field gains focus.
     *
     * @return the enter listeners for this text field
     */
    const std::vector<Listener> getEnterListeners() const;
     ...
     * @return the key for the listener
     */
    Uint32 addEnterListener(Listener listener);

    /**
     * Removes an enter listener from this text field.
     *
     * This listener is invoked when the field gains focus.
     *
     * Listeners must be identified by the key returned by the {@link #addExitListener}
     ...
     * @return true if the listener was succesfully removed
     */
    bool removeEnterListener(Uint32 key);
...
#pragma mark Internal Helpers
     ...
     *
     * Only one type of listener (enter, exit, type) can be invoked at at
     * time.
     *
     * @param listenerType  The listener type to invoke
     */
    void invokeListeners(TextField::ListenerType listenerType);
