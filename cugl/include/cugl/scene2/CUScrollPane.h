...
protected:
    ...
    std::shared_ptr<graphics::Scissor> _panemask;
...
#pragma mark Static Constructors
     ...
     */
    static std::shared_ptr<ScrollPane> allocWithData(const AssetManager* manager,
...
#pragma mark Attributes
    /**
     * Returns the transform applied to the children of this node
     *
     * This function returns the transform that is applied to children of this
     * node, including that due to the pane transformation.
     *
     * @return the transform applied to the children of this node
     */
    Affine2 getNodeToWorldTransform() const override;
     ...
     * (which in this case means the visible region of the scroll pane). In
     * addition, it will recursively check whether it is in the bound of its
     * ancestors. Only if the point is also in all of its ancestor's content
     ...
     */
    bool inContentBounds(const Vec2 point) override;
...
#pragma mark Rendering
     ...
     */
    virtual void render(const std::shared_ptr<graphics::SpriteBatch>& batch, const Affine2& transform, Color4 tint) override;
     ...
     * @param batch     The SpriteBatch to draw with.
     */
    virtual void render(const std::shared_ptr<graphics::SpriteBatch>& batch) override {
