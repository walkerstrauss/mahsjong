 ...
 * draw a bezier, create a {@link Path2} with the {@link SplinePather} factory.
 ...
 * in a level editor for your game. These methods determine the part of the bezier
...
#pragma mark Assignment Operators
     ...
     * @return This spline, returned for chaining
     */
    Spline2& operator=(const Vec2 point) { return set(point); }
     ...
     * @return This spline, returned for chaining
     */
    Spline2& set(const Vec2* points, size_t size);
...
#pragma mark Attribute Accessors
     ...
     * a closed spline has n anchors. The value index should be in the
     * appropriate range.
     *
...
#pragma mark Anchor Editting Methods
     ...
     */
    size_t addAnchor(const Vec2 point) { return addAnchor(point,point); }
     ...
     */
    void insertAnchor(float param)  { insertAnchor((size_t)param,param-(size_t)param); }
...
protected:
     ...
     *
     * de Castlejau's takes a parameter tp in (0,1) and splits the bezier into
     * two, preserving the geometric information, but not the parameterization.
     * The control points for the resulting two beziers are stored in left and
     * right.
     *
    ...
    void subdivide(size_t segment, float tp, std::vector<Vec2>& left, std::vector<Vec2>& rght) const {
        subdivide(_points.data()+3*segment,tp,left,rght);
    }
     ...
     * @param  rght     vector to store the right bezier
     */
    static void subdivide(const Vec2* src, float tp,
