...
public:
     ...
     * represents red or the color (1, 0, 0, 1). Because this representation
     * is endian specific, it is converted to the native endianness.
     *
     ...
     * @param color The integer to interpret as an RGBA value.
     */
	Color4f(const std::string color);
...
#pragma mark Setters
     ...
     * @return A reference to this (modified) Color4f for chaining.
     */
    Color4f& operator=(const std::string name) {
        return set(name);
    }
     ...
     * @return A reference to this (modified) Color4f for chaining.
     */
    Color4f& set(const std::string name);
     ...
     * @return A reference to this (modified) Color4f for chaining.
     */
    Color4f& setHSV(float h, float s, float v, float a=1);
     ...
     * @return A reference to this (modified) Color4f for chaining.
     */
    Color4f& setHSL(float h, float s, float l, float a=1);
...
public:
     ...
     *
     * This method returns the color as an integer in network order, which 
     * matches the way web colors are specified. In this order, red will 
     * always be the highest order byte and alpha will always be the lowest 
     * order byte. Hence 0x00ff00ff represents green or the color (0, 1, 0, 1).
     *
     * The value returned by this method is guaranteed to be the same value
     * that you assigned the constructor, or via setter. However, it is 
     * not guaranteed to be a value recognizable by OpenGL. For that value
     * you will need to call {@link #getPacked} instead.
     *
     * @return the canonical integer representation of this color
     */
     ...
     * correct one to send to OpenGL (which always uses client order).
     *
...
public:
union {
	// This favors little-ending (which is the standard)
    struct {
        /** The red value */
        Uint8 r;
        /** The green value */
...
public:
    ...
    */
     ...
     * Creates a new color from an integer interpreted as an RGBA value.
     *
     * This representation assigned is native to the platform, but the integer
     * parameter is interpreted in network order, which matches the way web 
     * colors are specified. In this order, red will always be the highest 
     * order byte and alpha will always be the lowest order byte. Hence  
     * 0x00ff00ff represents green or the color (0, 1, 0, 1).
     *
     * @param color The integer to interpret as an RGBA value.
     */
    Color4(Uint32 color) { set(color); }
...
#pragma mark Arithmetic
    ...
    Color4& scale(float s, bool alpha = false) {
        r = clampb(static_cast<Uint8>(r*s),0,255);
        g = clampb(static_cast<Uint8>(g*s),0,255);
        b = clampb(static_cast<Uint8>(b*s),0,255);
        a = (alpha ? clampb(static_cast<Uint8>(a*s),0,255) : a);
        return *this;
    }
    ...
    Color4& scale(float sr, float sg, float sb, float sa = 1) {
        r = clampb(static_cast<Uint32>(r*sr),0,255);
        g = clampb(static_cast<Uint32>(g*sg),0,255);
        b = clampb(static_cast<Uint32>(b*sb),0,255);
        a = clampb(static_cast<Uint32>(a*sa),0,255);
        return *this;
    }
    ...
    Color4& scale(Color4 c, bool alpha = false) {
         r = clampb(static_cast<Uint32>(COLOR_BYTE_TO_FLOAT(c.r)*r),0,255);
         g = clampb(static_cast<Uint32>(COLOR_BYTE_TO_FLOAT(c.g)*g),0,255);
         b = clampb(static_cast<Uint32>(COLOR_BYTE_TO_FLOAT(c.b)*b),0,255);
         a = (alpha ?  clampb(static_cast<Uint32>(COLOR_BYTE_TO_FLOAT(c.a)*a),0,255) : a);
         return *this;
    }
...
#pragma mark Operators
    ...
    Color4& operator*=(float s) {
        ...
        g = clampb(static_cast<Uint8>(g*s),0,255);
        b = clampb(static_cast<Uint8>(b*s),0,255);
        a = clampb(static_cast<Uint8>(a*s),0,255);
        return *this;
    }
     ...
     */
    Color4& operator*=(Color4 c) {
        r = clampb(static_cast<Uint8>(COLOR_BYTE_TO_FLOAT(c.r)*r),0,255);
        g = clampb(static_cast<Uint8>(COLOR_BYTE_TO_FLOAT(c.g)*g),0,255);
        b = clampb(static_cast<Uint8>(COLOR_BYTE_TO_FLOAT(c.b)*b),0,255);
        a = clampb(static_cast<Uint8>(COLOR_BYTE_TO_FLOAT(c.a)*a),0,255);
        return *this;
    }
