 ...
 * used to interpolate the function over time (e.g. for tweening support). The
 * function retains a shared pointer to the object, so the object reference can
 * be safely discarded after getting the function pointer.
 */
...
public:
     ...
     */
    bool init() {
        return init(EasingFactory::Type::LINEAR);
    }
     ...
     * @return true if initialization was successful.
     */
    bool init(EasingFactory::Type type);
...
#pragma mark Static Constructors
     ...
     */
    static std::shared_ptr<EasingBezier> alloc(EasingFactory::Type type) {
        std::shared_ptr<EasingBezier> result = std::make_shared<EasingBezier>();
...
#pragma mark Easing Support
     ...
     */
    EasingFunction getEvaluator();
    
};
    
}

#endif /* __CU_EASING_BEZIER_H__X */
