...
#pragma mark Listeners
     ...
     *
     * There can only be one key down listener for a given key (though you
     * may share keys across other listener types). If a listener already
     * exists for the key, the method will fail and return false. You must
     * remove a listener before adding a new one for the same key.
     *
     * This listener is invoked when a key is pressed.
     *
     * @param key       The identifier for the listener
     ...
     *
     * There can only be one key up listener for a given key (though you
     * may share keys across other listener types). If a listener already
     * exists for the key, the method will fail and return false. You must
     * remove a listener before adding a new one for the same key.
     *
