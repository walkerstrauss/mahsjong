 ...
 *
 * This class uses CUJSON as the underlying parsing engine.  However, it manages
...
#pragma mark -
#pragma mark CUJSON Conversions
    /**
     ...
     */
    static std::shared_ptr<JsonValue> toJsonValue(const CUJSON* node);
     ...
     * This method does not delete the CUJSON node when done.
     *
     * @param value The JsonValue to store the result
     * @param node  The CUJSON node to convert
     */
    static void toJsonValue(JsonValue* value, const CUJSON* node);
     ...
     * it must be manually deleted (with `CUJSON_Delete`) when it is no
     * longer necessary.
     *
     * @param value The JsonValue to convert
     */
    static CUJSON* toCUJSON(const JsonValue* value);
...
#pragma mark Child Access
     ...
     * 
     * This value is non-empty only if this node is an array or an object.
     *
     * @return a reference to the children of this node 
     */
    const std::vector<std::shared_ptr<JsonValue>>& children() { return _children; }
...
#pragma mark Child Addition
	 ...
	 */
	void merge(const std::shared_ptr<JsonValue>& node);
