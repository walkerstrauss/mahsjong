...
protected:
     ...
     * previously loaded assets (e.g. scene graphs). In the current
     * architecture, this method is only correct if the asset manager loads
     * assets in a single thread.
     */
    void sync();
...
#pragma mark Directory Support
     ...
     *
     * Some loaders depend upon each other. For example {@link scene2::Scene2Loader}
     * typically requires {@link graphics::TextureLoader} to finish loading all
     * textures first. To support this relationship, loaders have priorities, 
     * given by {@link BaseLoader#getPriority}. Loaders with higher priority are
