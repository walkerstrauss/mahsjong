...
//
//  This module provides support for an offscreen render target.  That is, a 
//  framebuffer with (potentially multiple) attached output buffers.  This
//  allows us to draw to a texture for potential post processing.
//  
...
#pragma mark Attributes
     ...
     *
     * The framebuffer object is the OpenGL object that stores the output
     * textures and depth/stencil buffer.  It is created when the render
     * target is initialized.
     *
     * This function should only be necessary for the Vulkan renderer, as Vulkan::begin requires
     * a framebuffer object if you wish to render to a different render target than the swapchain.
     *
     * @return the framebuffer object for this render target
     */
    GLuint getFramebuffer() const { return _framebo; }
