...
#ifdef CU_VULKAN
        /**
         * A texture used to store RGBA data.
         *
         * This is a specific internal format used for certain Vulkan implementations
         (namely {@link RenderTarget}). This format uses GL_STENCIL_INDEX8 as the internal
         * format. The data type is GL_UNSIGNED_BYTE.
         */
        BGRA = GL_BGRA,
#endif
        /**
...
public:
    ...
    bool init(int width, int height, PixelFormat format = PixelFormat::RGBA) {
        return initWithData(NULL, width, height, false, format);
    }
     ...
     * @param width     The texture width in pixels
     * @param height    The texture height in pixels
     * @param mipmaps   Whether or not to generate mipmaps
     * @param format    The texture data format
     *
     * @return true if initialization was successful.
     */
    bool initWithData(const void *data, int width, int height,
                      bool mipmaps = false, PixelFormat format = PixelFormat::RGBA);
     ...
     * @return true if initialization was successful.
     */
    bool initWithFile(const std::string filename, bool mipmaps = false);
...
#pragma mark Static Constructors
    ...
    static std::shared_ptr<Texture> alloc(int width, int height,
        ...
        return (result->init(width, height, (Texture::PixelFormat)format) ? result : nullptr);
    }
    ...
    static std::shared_ptr<Texture> allocWithData(const void *data, int width, int height,
                                                  bool mipmaps = false, PixelFormat format = PixelFormat::RGBA) {
        std::shared_ptr<Texture> result = std::make_shared<Texture>();
        return (result->initWithData(data, width, height, mipmaps, (Texture::PixelFormat)format) ? result : nullptr);
     ...
     * @return a new texture with the given data
     */
    static std::shared_ptr<Texture> allocWithFile(const std::string filename, bool mipmaps = false) {
        std::shared_ptr<Texture> result = std::make_shared<Texture>();
        return (result->initWithFile(filename, mipmaps) ? result : nullptr);
    }
