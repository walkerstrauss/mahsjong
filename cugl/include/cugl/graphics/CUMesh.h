 ...
 *
 * A mesh is a collection of vertices, together with indices and a drawing
 * command. The type of the indices and drawing command are fixed, but the
 * vertex type is templated. This allows a mesh to be adapter to an arbitrary
 * {@link Shader}.
 *
 * The only requirement of a mesh vertex is that it have at least one field
 * called position, and this type be one of {@link Vec2}, {@link Vec3} or
 * {@link Vec4}.
 */
template <typename T>
class Mesh {
...
#pragma mark Setters
     ...
     *
     * No vertex attribute other than position is set. Additional information
     * (such as color or texture coordinate) must be added later. The command
     ...
     * This method returns a reference to this polygon for chaining.
     *
     * @param path  The path defining this mesh
     *
     * @return This mesh for chaining
     */
     ...
     * No vertex attribute other than position is set. Additional information
     * (such as color or texture coordinate) must be added later. The command
     * will be GL_TRIANGLES if the polygon is solid, GL_LINES if it is a path,
     * and GL_UNDEFINED otherwise.
     *
     * This method returns a reference to this polygon for chaining.
     *
     * @param poly  The polygon defining this mesh
...
/**
 * Triangulates the mesh with the given triangulation algorithm
 *
 * This function is used to populate the indices in a mesh.
 *
 * @tparam A    The triangulator class/algorithm
 * @tparam T    The vertex type for the mesh
 * @param mesh  The index set to receive the results
 */
template <typename A, typename T>
static void triangulate_mesh(Mesh<T>& mesh) {
    A triangulator;
    ...
    triangulator.getTriangulation(mesh.indices);
}

/**
 * Populates a the mesh with the date in the given JsonValue.
 *
 * For this template function to work, the type T must have a constructor
 * that takes a JsonValue to initialize an individual vertex. Note that
 * {@link SpriteVertex} has such a constructor.
 *
 * The mesh can be represented as either an array or JSON object. If it is
 * an array, it then it should contain JSON values compatible with the
 * constructor for that type. If it is a JSON object, then it supports the
 ...
 * @param json  The JSON object specifying the mesh
 */
template <typename T>
void parse_mesh(Mesh<T>& mesh, const std::shared_ptr<JsonValue>& json) {
    JsonValue* verts = nullptr;
    if (json->isArray()) {
        verts = json.get();
    } else if (json->has("vertices")) {
        verts = json->get("vertices").get();
    }
    
    if (verts != nullptr) {
        for(auto it = verts->children().begin(); it != verts->children().end(); ++it) {
            auto child = *it;
            mesh.vertices.emplace_back(child);
        }
    }
    ...
    if (json->isObject()) {
        if (json->has("indices")) {
            JsonValue* idxs = json->get("indices").get();
            CUAssertLog(idxs->isArray(), "Index set %s is not an array", idxs->toString().c_str());
            for(auto it = idxs->children().begin(); it != idxs->children().end(); ++it) {
                JsonValue* child = it->get();
                mesh.indices.push_back(child->asInt());
            }
        } else if (json->has("triangulator")) {
            std::string choice = json->getString("triangulator","fan");
            if (choice == "fan") {
                for(int ii = 0; ii < mesh.vertices.size(); ii++) {
                    mesh.indices.push_back(ii);
                }
                mesh.command = GL_TRIANGLE_FAN;
            } else if (choice == "strip") {
                for(int ii = 0; ii < mesh.vertices.size(); ii++) {
                    mesh.indices.push_back(ii);
                }
                mesh.command = GL_TRIANGLE_STRIP;
            } else {
                if (choice == "monotone") {
                    triangulate_mesh<EarclipTriangulator,T>(mesh);
                } else if (choice == "delaunay") {
                    triangulate_mesh<DelaunayTriangulator,T>(mesh);
                } else {
                    triangulate_mesh<EarclipTriangulator,T>(mesh);
                }
