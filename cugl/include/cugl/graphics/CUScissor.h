 ...
 * fringe is expanded into a per-axis value for the shader). And this is the 
 * format that this data is represented in the {@link getData} method so that
 * it can be passed to a {@link UniformBuffer} for improved performance. It is
 * also possible to get access to the individual components of the scissor mask,
 * to pass them to a shader directly (though the transform must be inverted
 * first if it is passed directly).
 *
 ...
 * The affect is the same as the operation {@link Rect#intersect}.
 */
class Scissor {
#pragma mark Values
private:
