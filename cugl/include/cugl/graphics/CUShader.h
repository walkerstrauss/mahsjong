...
//
//  Note that to use a shader, it must be attached to a VertexBuffer.
//
...
//
//  Author: Walker White, Zach Schnecter
 ...
 * agnostic about whether we are using OpenGL or OpenGLES. It is reasonably
 * lightweight enough to be general purpose. However, it only supports basic
 * vertex buffers and 2d textures. If you need anything more than this, you
 * may need to create your own shader class.
 *
 * Note that a shader cannot be used by itself. To use a shader, it must first
 * be attached to a {@link VertexBuffer}. When using a shader, with a vertex
 * buffer keep in mind the "performance hit" hierarchy. From our experiments,
 * the cost of changing data in a rendering phase going from most expensive to
 * cheapest is as follows:
 *
 *  <ul>
 *    <li>Render target</li>
 *    <li>Shader</li>
 ...
 *  </ul>
 *
 * By vertex stream we mean the act of loading data into a vertex buffer. Since
 * the cost of swapping a vertex buffer is more expensive that reloading data
 * into it, a vertex buffer should only be swapped when the format of the
 * vertex data changes.
 *
 * Uniforms tend to be fairly cheap. However, some uniforms are samplers and
 * those are incredibly expensive to change. A sampler is a uniform that is
 * bound to a texture bind point. Because of how textures are loaded, it is
 * (much) cheaper to activate a texture to an existing bind point than it is to
 * change the bind point used in the shader.
 *
 * Even the most basic uniforms are by no means cheap. The best case graphics
 * performance is when you can load the vertex buffer once and then call a
 * single draw command for all of the vertices (the difference is an order of
 * magnitude). Therefore, any properties that space only a few vertices (e.g.
 * quads) should be pushed into the vertex data itself. For example, it is
 * faster to transform quads in the CPU than it is in the GPU.
 *
 ...
 * binding of multiple output locations (glBindFragDataLocation). If a shader
 * has multiple output targets, then these must be explicitly managed inside
 * the shader with the layout keyword. Otherwise, the output bind points from
 * the appropriate query methods.
 */
class Shader {
#pragma mark Values
protected:
...
public:
     ...
     */
    static std::shared_ptr<Shader> alloc(const std::string vsource, const std::string fsource) {
...
#pragma mark Pipeline State
    // These methods abstract out OpenGL features for Vulkan transition
    /**
     ...
     */
    void setBlendFuncSeperate(GLenum srcRGB, GLenum dstRGB,
