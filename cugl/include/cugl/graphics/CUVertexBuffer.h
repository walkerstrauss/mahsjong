 ...
 * not have a true many one relationship with a {@link Shader} object. A vertex
 * buffer can only be connected to one shader at a time and vice versa. So we
 * model this as a direct connection. As vertex buffers push data to a shader,
 * the dependency requires that a shader be linked to a vertex buffer object.
 *
...
#pragma mark Vertex Processing
     ...
     *
     * The size determines the number of elements that can be loaded with either
     * {@link #loadVertexData} or {@link #loadIndexData}.
     *
     * @return the maximum capacity of this buffer.
     */
    GLsizei getCapacity() const { return _size; }
     ...
     *
     * The indices loaded are those that will be used at the next call to
     * {@link #draw}. Frequent reloading of indices is to be discouraged
     * (though it is faster than swapping to another vertex buffer). Instead,
     * indices should be loaded once (if possible) and draw calls should make
     * use of the offset parameter.
     *
     * The indices loaded are expected to refer to valid vertex positions. If
     * they do not, strange things will happen.
     *
     * The usage is one of GL_STATIC_DRAW, GL_STREAM_DRAW, or GL_DYNAMIC_DRAW.
     * Static drawing should be reserved for vertices and/or indices that do not
     * change (so all animation happens in uniforms). Given the high speed of
...
#pragma mark Attributes
     ...
     */
    virtual void setupAttribute(const std::string name, GLint size, GLenum type,
     ...
     * required by the shader, it will use the default value for the type
     * instead.
     *
