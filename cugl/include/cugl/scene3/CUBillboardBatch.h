...
//
#ifndef __CU_BILLBOARD_BATCH_H__
#define __CU_BILLBOARD_BATCH_H__
...
/**
 * This class is a batch for drawing {@link BillboardNode} objects.
 *
 * This class is only designed for {@link BillboardNode} objects. Attempts to
 ...
 */
class BillboardBatch : public Scene3Batch {
private:
    /**
     * The queue entry for an BillboardBatch.
     *
    ...
    public:
        /** The node to draw */
        std::shared_ptr<BillboardNode> node;
         ...
         */
        Entry(const std::shared_ptr<BillboardNode>& n, const Mat4& mat) {
            node = n;
         ...
         */
        void draw(const std::shared_ptr<BillboardShader>& shader);
    ...
    std::shared_ptr<BillboardShader> _shader;
    /** The batch queue */
    std::vector<Entry> _entries;
    
public:
    /** The key for this batch type */
    static const Uint32 BATCH_KEY = 2;
    ...
    /**
     * Initializes a new billboard batch with the default key and priority.
     *
    ...
    /**
     * Returns a newly allocated billboard batch with the default key and priority.
     *
     * This method can fail is the shader fails to compile.
     *
     * @return a newly allocated billboard batch with the default key and priority.
     */
    static std::shared_ptr<BillboardBatch> alloc() {
        std::shared_ptr<BillboardBatch> result = std::make_shared<BillboardBatch>();
        return (result->init() ? result : nullptr);
    }

    /**
     * Returns a newly allocated billboard batch with the given key and priority.
     *
     * This method can fail is the shader fails to compile.
     *
     * @param key       The batch key
     * @param priority  The batch priority
     *
     * @return a newly allocated billboard batch with the given key and priority.
     */
    static std::shared_ptr<BillboardBatch> alloc(Uint32 key, Uint32 priority) {
        std::shared_ptr<BillboardBatch> result = std::make_shared<BillboardBatch>();
     ...
     *
     * The scene node will be ignored if is not an instance of {@link BillboardNode}.
...
#endif /* __CU_BILLBOARD_BATCH_H__ */
