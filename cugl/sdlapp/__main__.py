...
After years of supporting student projects, we got tired of supporting all the 
...
Originally, we had hoped CMake would solve the problem. However, CMake is an 
absolute mess when it comes to mobile builds. While there are some experimental 
iOS generators, they are not reliable and they completely fall apart with 
respect to code signing. The situation is even worse on Android where there is 
no generator support for Android Studio, and CMake cannot modify any of the Java 
configuration files (which is where most of the application specific issues are). 
So while CMake is fine on desktop platforms (and is the ideal build system for 
Linux), we had to abandon it.

This Python script is the compromise. For a given SDL project, we generate the 
project files for each target platform. It is designed so that users can safely 
edit code without regenerating the project files. However major changes to the 
project (even something as simple as making a new icon) should rebuild the 
project files.

To work properly, the SDL project must have a config.yml in its root directory.
...
def snake2Camel(text):
    ...
    All underscores are removed, and capital letters are placed at underscore 
    boundaries.
